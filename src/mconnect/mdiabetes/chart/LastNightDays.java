 ...
 */
public class LastNightDays extends AbstractChart {
  /**
   ...
   */
	...
	public LastNightDays(int y,int m,int d){
		this._y=y;
		this._m=m;
		this._d=d;
	}
  ...
  public Intent execute(Context context) {
    ...
    setChartSettings(renderer, "过去90天的血压分布", "时间", "血压值(mmHg)", 0.5, 10.51, 0, 300,
    	    Color.GREEN, Color.GREEN);
    	renderer.setXLabels(1);//表示横坐标被分为多少等分
    	CLastNightDays clw=new CLastNightDays();
    	String []we=new String[91];
    	we=clw.getlastNightDays(_y,_m,_d);
    	
    	renderer.setYLabels(12);//表示纵坐标被分为多少等分
    	for(int i=0;i<10;i++)
    	{
    		renderer.addTextLabel(i+1, we[i*10]);
    	}
    	
    	//renderer.addTextLabel(10, "");
