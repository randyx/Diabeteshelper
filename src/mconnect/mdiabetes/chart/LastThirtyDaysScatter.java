 ...
 */
public class LastThirtyDaysScatter extends AbstractChart {
  /**
   ...
   */
	...
	public LastThirtyDaysScatter(int y,int m,int d){
		this._y=y;
		this._m=m;
		this._d=d;
	}
  ...
  public Intent execute(Context context) {
    ...
    setChartSettings(renderer, "过去30天的血压分布", "时间", "血压值(mmHg)", 0.5, 7.5, 0, 300,
    	    Color.GREEN, Color.GREEN);
    	renderer.setXLabels(1);//表示横坐标被分为多少等分
    	CLastThirtyDays clw=new CLastThirtyDays();
    	String []we=new String[31];
    	we=clw.getThirtyDay(_y,_m,_d);
    	
    	renderer.setYLabels(12);//表示纵坐标被分为多少等分
    	for(int i=0;i<7;i++)
    	{
    		renderer.addTextLabel(i+1, we[i*5]);
    	}
